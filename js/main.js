    ...
    <article class="product-card" data-categorie="${produit.categorie}" data-prix="${getPrixRange(produit)}" data-bestseller="${produit.bestSeller ? 'true' : 'false'}">
      ...
      <div class="product-card__body">
        ...
        <div class="product-card__footer">
          <span class="product-card__prix">${produit.prix > 0 ? produit.prix.toLocaleString() + ' ' + (produit.devise || 'FCFA') : '<em style="font-size:var(--text-xs);color:var(--color-text-muted);">Sur demande</em>'}</span>
          <div style="display:flex;gap:4px;align-items:center;flex-shrink:0;">
            <button class="btn btn--panier btn--sm" onclick="event.preventDefault();Panier.ajouter(${JSON.stringify({id:produit.id,nom:produit.nom,marque:produit.marque||'',prix:produit.prix,devise:produit.devise||'FCFA',images:produit.images,emoji:produit.emoji||'🛍️'}).replace(/"/g,"'").replace(/'/g,'&apos;')})" aria-label="Ajouter au panier" title="Ajouter au panier">🛍️</button>
            <a href="${getWhatsAppLink(produit)}" class="btn btn--whatsapp btn--sm" target="_blank" rel="noopener" style="padding:6px 10px;font-size:0.7rem;gap:4px;">
